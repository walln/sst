...
Autodeploy is free while in beta. But AWS will charge you for the CodeBuild build minutes that are used. [Learn more about CodeBuild pricing](https://aws.amazon.com/codebuild/pricing/).
...
This only applies only to git events. If you trigger a deploy through the Console, you are asked to sepcify the stage you want to deploy to. So in this case, it skips step 1 from above and does not call `console.autodeploy.target`.
