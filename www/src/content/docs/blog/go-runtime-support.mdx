...
SST v3 now supports the [`go`](/docs/component/aws/function#runtime) runtime for your Lambda functions. Golang is a great option for tasks that are more compute intensive and it also has faster cold starts.

<YouTube id="WTAeW0wyTzA" posterQuality="high" />

You can use it by setting the `runtime` prop and pointing the `handler` to the directory with your Go function.
